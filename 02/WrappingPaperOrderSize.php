...
{
    public function calcWrappingPaper()
    {
    ...
    public function calcRibbon()
    {
        ...
        foreach ($inputs as $input) {
            ...
            asort($dms);
            $wp = array_slice($dms, 0, 2);

            $wrap = ($wp[0] + $wp[0]) + ($wp[1] + $wp[1]);

            $bow = $dms[0]  * $dms[1] * $dms[2];
            $total_sqf += $bow + $wrap;
        }
        echo $total_sqf . PHP_EOL;
    }
...
(new WrappingPaperOrderSize())->calcRibbon();
